...
<?php $setting_supportedLocales = ($form->setting_supportedLocales !='') ? array_flip(json_decode($form->setting_supportedLocales)) : null;?> 
<div class="form-group row">
    <label for="setting_defaultLocale" class="col-xl-3 col-lg-3 col-form-label"><?= ucfirst(lang('Core.language_supported')); ?>* : </label>
    <div class="col-lg-9 col-xl-6">
        <select  <?= ($form->setting_activer_multilangue == true) ? 'required' : ''; ?> name="global[setting_supportedLocales][]" class="form-control kt-selectpicker" multiple title="<?= ucfirst(lang('Core.choose_one_of_the_following')); ?>" id="setting_supportedLocales">
            <?php foreach ($languages as $language) { ?>
